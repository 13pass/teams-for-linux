...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `graphApi.enabled` | `boolean` | `false` | Enable the Microsoft Graph API integration for calendar, mail, and chat/channel access. |
| `graphApi.probeMessageAccess` | `boolean` | `false` | After sign-in, log a one-time probe of chat/channel message-read access so you can see whether the borrowed Teams token carries the required Graph scopes (Chat.Read, ChannelMessage.Read.All). Requires enabled. No message content is logged. |
